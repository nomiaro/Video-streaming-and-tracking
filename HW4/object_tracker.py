...
def main(_argv):
    ...
    if not FLAGS.dont_show:
        ...
        cv2.resizeWindow("Output Video", 960, 540)
    ...
    process = (
        ...
        .output(os.path.join(m3u8_path, 'try.m3u8'), format = 'hls', pix_fmt = 'yuv420p', vcodec = 'libx264', preset = "ultrafast", hls_time = 10, hls_list_size = 2, start_number = 0, hls_flags = 'delete_segments+append_list', hls_segment_filename = '../try_%03d.ts')
        .overwrite_output()
    ...
    streamer = StreamGear(output=os.path.join(m3u8_path, "hls_out.m3u8"), format = "hls", **stream_params)
    ...
    while True:
        ...
        for track in tracker.tracks:
            if not track.is_confirmed() or track.time_since_update > 1:
                continue
            bbox.append(track.to_tlbr())
            ID_list.append(track.track_id)
        param = {}
        param['bbox'] = bbox
        param['ID_list'] = ID_list
        cv2.setMouseCallback("Output Video", mouse_event, param)
